...
//! stream can outlive any one session, and the LinkStream adapter that
//! pulls samples out of the presented core of a live link and resamples
//! to the host rate.
...
impl Stream for LinkStream {
    fn fill(&mut self, buf: &mut [[i16; NUM_CHANNELS]]) -> usize {
        ...
        let player = self.shared.view_player.load(std::sync::atomic::Ordering::Relaxed);
        ...
        let pulled = self.access.with_link(|link| {
            ...
            let faux_clock = link.core(player).calculate_framerate_ratio(fps_target as f64);
