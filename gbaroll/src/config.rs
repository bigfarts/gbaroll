...
pub struct Config {
    ...
    /// Each game's last-picked save (game CRC32 → file name inside
    /// its `saves/<crc32>/` directory), re-selected when the game is.
    /// No entry = the fresh-save row.
    pub last_saves: HashMap<u32, String>,
