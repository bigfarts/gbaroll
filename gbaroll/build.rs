//! Compiles the audio worklet's DSP module (../gbaroll-worklet) so
...
fn main() {
    println!("cargo:rerun-if-changed=../gbaroll-worklet/src");
    println!("cargo:rerun-if-changed=../gbaroll-worklet/Cargo.toml");
    ...
    let status = Command::new(&cargo)
        ...
        .current_dir("../gbaroll-worklet")
        .env("CARGO_TARGET_DIR", &target_dir)
        // The parent build's flags are for the parent build.
        .env_remove("RUSTFLAGS")
        .env_remove("CARGO_ENCODED_RUSTFLAGS")
        .status()
        .expect("couldn't spawn cargo for gbaroll-worklet");
    assert!(status.success(), "gbaroll-worklet build failed");

    let wasm = target_dir.join("wasm32-unknown-unknown/release/gbaroll_worklet.wasm");
    std::fs::copy(&wasm, out_dir.join("gbaroll_worklet.wasm")).expect("couldn't copy worklet wasm");
}
