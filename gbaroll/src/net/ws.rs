//! The signaling websocket, browser flavor: `web_sys::WebSocket` in
//! arraybuffer mode, bridged to an async pull API. `connect` resolves
//! once the socket opens; incoming binary frames queue in an unbounded
//! channel; the channel closing means the socket closed.
